...
| `lineups` | festival × artist join | FK `festival_id`, `artist_id`; `year`; `source`; unique `(festival_id, artist_id, year)` | `day` + `set_time_*` drive the schedule views (set times are **local** to `festivals.timezone`). `source` = provenance/trust; a `protect_verified_lineups` trigger stops scrapers overwriting `official`/`wikipedia` rows. |
